...
## 設定オプション (CloudVarOptions)

インスタンス生成時に指定できるオプションです。IDEの補完機能により、これらのオプションは自動的にサジェストされます。

```javascript
const cv = new CloudVar('ws://localhost:5032', {
  room: 'lobby',      // 参加するルームID
  token: 'my-token',  // 認証トークン
  name: 'player1'     // クライアント名（指定するとグローバル変数の自動展開を行いません）
});
```

- **room**: 同期するルームID。
- **token**: サーバーへの認証に使用するトークン。
- **name**: このインスタンスを一意に識別する名前。指定した場合、`window` オブジェクトへのグローバル変数の自動注入を行いません（純粋なJSオブジェクトとして扱いたい場合に便利です）。
